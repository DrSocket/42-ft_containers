	...
	struct RBT_Node {
		...
		/* member function: constructor / destructor / operator=  */
			...
			RBT_Node(value_type const & val,
				...
				RBT_Node *parent = nullptr_t,
				RBT_Node *left = nullptr_t,
				RBT_Node *right = nullptr_t) :
					val(val),
					color(color),
					parent(parent),
					left(left),
	...
	class RBT {
		...
		/* member function: modifiers  */
			...
			bool	insert(value_type const & val) {
				...
				allocator_type().construct(toInsert, node_type(val, RED_NODE, nullptr_t, _null, _null)); // new node must be red

				pointer current = nullptr_t;
				pointer root = _root;
				...
				if (toInsert->parent == nullptr_t) {
					toInsert->color = BLACK_NODE;
					return true;
				}

				if (toInsert->parent->parent == nullptr_t)
					return true;
		...
		/* member function: iteration  */
			...
			pointer	successor(pointer node) const {
				...
				if (node->parent == nullptr_t || node == max())
					return _null;
			...
			pointer	predecessor(pointer node) const {
				...
				if (node->parent == nullptr_t || node == min())
					return _null;
				pointer	tmp = node->parent;
